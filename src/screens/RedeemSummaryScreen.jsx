...
import { useGetRedeemHistoryByVendorQuery } from '../features/shops/shopApi';
...
const RedeemSummaryScreen = ({ route }) => {
    ...
    const handleRedeemConfirm = async (id) => {
        ...
        try {
            ...
            setMessage(error?.data?.message || 'Failed to redeem points. Please try again.');
            setTimeout(() => setMessage(null), 2000); 
        }
    };
    ...
    return (
        ...
        >
            ...
            <ScrollView contentContainerStyle={styles.container}>
                ...
                <LinearGradient colors={['#DA22FF', '#9733EE']} style={styles.balanceCard}>
                    <Text style={styles.balanceLabel}>POINT BALANCE</Text>
                    <Text style={styles.balanceAmount}>{vendorDetails.points.balanceVendorPoints.toFixed(2)}</Text>
                ...
                {Array.isArray(data?.data) && data.data.length > 0 ? (data.data.map(item => (
                            ...
                            <View style={styles.modalButtons}>
                                ...
                                <TouchableOpacity
                                    style={[
                                        styles.modalBtn,
                                        {
                                            backgroundColor: redeemPoints || isDisabled ? '#DA22FF' : '#888',
                                        },
                                    ]}
